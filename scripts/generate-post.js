...
async function generatePost() {
    ...
    if (process.argv[2]) {
        ...
        try {
            ...
            🚨 [중요 가이드라인] 소화기 질환(위·대장 내시경) 및 건강검진 주제를 80% 이상 최우선으로 선정하세요:
            - [최우선] 소화기 질환 및 내시경: 위염, 위궤양, 역류성 식도염, 대장 용종, 조기 위암/대장암 예방, 헬리코박터 제균, 장상피화생, 과민성 대장 증후군, 만성 소화불량, 올림푸스 CV-290 정밀 내시경 및 당일 용종 절제술
            - [우선] 건강검진: 국가 5대 암검진, 연령별 맞춤 종합검진, 검진 전 주의사항, 초음파 검진
            - [보조] 만성질환 및 내과 일반: 고혈압, 당뇨병, 고지혈증, 지방간, 갑상선 질환
            - [금지/제한] 뇌 CT, 두통 CT, 심장 CT 등 뇌/심장 CT 단독 주제는 지양하고, 오직 소화기(복부/간/담낭/췌장) 정밀 검사 맥락에서만 필요시 다루세요.

            🚨 절대 사용 금지 키워드: '대학병원', '대학병원급', '대학병원처럼' (의료광고법 위반 소지가 있어 절대 사용해서는 안 됩니다. 대신 '대형병원', '종합병원' 등으로 대체해서 기획하세요.)
...
## 새로운 글쓰기 패턴 (매우 중요)
- **주제 내용**: [진료 경험에서 본 패턴] 1~2개를 반드시 포함하세요.
- **형식**: 자유롭게 구성하되 (질문 도입, 사례 도입, 통계 도입, 비교 도입) 중 랜덤으로 하나를 선택하여 시작하세요. 기계적인 H2 구조(원인, 증상, 치료 등)를 탈피하세요.
- **어조**: 의학 정보를 전달하는 객관적이고 중립적인 해설자 톤을 유지하세요. '저희', '저는', '의료진' 등 화자를 지칭하는 단어를 일절 사용하지 마세요.
- **메타 디스크립션**: 환자가 실제로 검색할 만한 검색어 기준으로 작성하고, 클리닉명(병원명)은 **절대 포함하지 마세요**.
- **글 끝부분 마무리**: 글의 마지막에는 자연스럽게 [진료 가능 항목]을 포함하여 신뢰도를 높이세요.
